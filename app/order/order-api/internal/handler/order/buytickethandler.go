...
func BuyTicketHandler(svcCtx *svc.ServiceContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		var req types.BuyTicketReq
		if err := httpx.Parse(r, &req); err != nil {
			response.HttpResult(w, r, nil, err)
			return
		}

		l := order.NewBuyTicketLogic(r.Context(), svcCtx)
		resp, err := l.BuyTicket(&req)
